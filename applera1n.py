...
def show_popup():
    ...
    button1 = tk.Button(
        popup, text="build fake fs", command=build_fakefs, width=20, height=5
    )
    button1.pack(pady=10)

    button2 = tk.Button(
        popup, text="boot fake fs", command=boot_fakefs, width=20, height=5
    )
    button1.pack(pady=10)
    button2.pack()
...
def show_popup2():
    ...
    # create larger buttons
    button1 = tk.Button(
        popup, text="bypass ios 15-16.3", command=startbypass, width=20, height=5
    )
    button1.pack(pady=10)

    button2 = tk.Button(
        popup, text="bypass ios 15-16.6", command=icloudbypassios16, width=20, height=5
    )
    button1.pack(pady=10)
    button2.pack()
...
def build_fakefs():
    messagebox.showinfo(
        "",
        "Please put the device first in recovery mode and then into dfu mode after the device is in dfu click ok",
    )
    ...
    if response == 1:  # Yes
        ...
        if system_name == "posix":
            process = subprocess.Popen(
                ["./device/Darwin/palera1n", "-cf"], preexec_fn=os.setsid
            )
        elif system_name == "darwin":
            process = subprocess.Popen(
                ["./device/Linux/palera1n", "-cf"], preexec_fn=os.setsid
            )
...
def boot_fakefs():
    ...
    if response == 1:  # Yes
        ...
        if system_name == "posix":
            process = subprocess.Popen(
                ["./device/Darwin/palera1n", "-f"], preexec_fn=os.setsid
            )
        elif system_name == "darwin":
            process = subprocess.Popen(
                ["./device/Linux/palera1n", "-f"], preexec_fn=os.setsid
            )
        # Wait for 8 seconds or until the process completes
        for i in range(20):
            if process.poll() is not None:
...
def icloudbypassios16():
    ...
    showinfo(
        "bypass!",
        "We will now bypass your device! please jailbreak first with palera1n C!",
    )
    if system_name == "posix":
...
def startbypass():
    ...
    if "ERROR:" in fileData:
        ...
        if len(UDID) > 38:
            ...
            messagebox.showinfo(
                "iDevice is detected!", "Found iDevice on iOS " + LAST_CONNECTED_IOS_VER
            )
    ...
    if len(LAST_CONNECTED_IOS_VER) < 2:
        ...
        showinfo(
            "Ready to Jailbreak...",
            "Hi, iOS "
            + str(LAST_CONNECTED_IOS_VER)
            + ". \n\nWe will now bypass iOS "
            + str(LAST_CONNECTED_IOS_VER)
            + " Semi-Tethered.",
        )
        ...
        os.system("idevicepair pair")
        os.system(
            f"cd ./palera1n/ && ./palera1n.sh --tweaks --semi-tethered {LAST_CONNECTED_IOS_VER}"
        )
...
# label
my_label2 = Label(frame, text="Designed to run on devices on ios 15-16.6")
my_label2.place(x=130, y=130)

# label
my_label3 = Label(frame, text="ver 1.6")
my_label3.place(x=10, y=220)


cButton1 = tk.Button(frame, text="start bypass", command=show_popup2, state="normal")
cButton1.place(x=200, y=160)
cButton2 = tk.Button(frame, text="clear files", command=clear, state="normal")
cButton2.place(x=340, y=160)
cButton3 = tk.Button(frame, text="enter recovery", command=enterRecMode, state="normal")
cButton3.place(x=10, y=10)
cButton4 = tk.Button(frame, text="exit recovery", command=exitRecMode, state="normal")
cButton4.place(x=380, y=10)
cButton5 = tk.Button(frame, text="start palera1nC", command=show_popup, state="normal")
cButton5.place(x=50, y=160)
# Create a Label to display the link
link = Label(
    root, text="Made this tool @hackt1vator", font=("Helveticabold", 12), cursor="hand2"
)
link.place(x=165, y=220)
link.bind("<Button-1>", lambda e: callback("https://twitter.com/hackt1vator"))

cbeginExploit2 = tk.Button(frame, text="Twitter!", command=opentwitter, state="normal")
